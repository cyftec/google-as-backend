...
export class GisAuth {
  ...
  private ensureGsiScriptLoaded(): Promise<void> {
    ...
    if (!GisAuth.gsiScriptLoadPromise) {
      GisAuth.gsiScriptLoadPromise = GisAuth.loadGsiScript().catch((error) => {
        GisAuth.gsiScriptLoadPromise = null;
        throw error;
      });
    }

    return GisAuth.gsiScriptLoadPromise;
  }
  ...
  private static loadGsiScript(): Promise<void> {
    ...
    if (existingScript) {
      return GisAuth.waitForGsiScript(existingScript);
    }
    ...
    return GisAuth.waitForGsiScript(script);
  }
